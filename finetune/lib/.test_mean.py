...
def crf(pred, coords, input, target):
  theta_p = 2
  theta_c = 0.3
  weight_p = 1
  weight_c = 1
  ...
  coords = coords[:, 1:]
  pred = softmax(pred)
  coord_neigh = NearestNeighbors(n_neighbors=100, algorithm='auto').fit(coords)
  for idx in range(5):
    print(idx)
    o2 = pred.copy()
    for i in range(pred.shape[0]):
      pcoord = coords[i, :]
      pcoord = pcoord.reshape((1 , -1))
      pcolor = input[i, :]
      coord_dist, coord_indexes = coord_neigh.kneighbors(pcoord, return_distance=True)
      localcoords = coords[coord_indexes[0,:]]
      localcolor = input[coord_indexes[0,:]]
      color_dist = pcolor - localcolor
      color_dist = np.linalg.norm(color_dist, ord=2, axis=1, keepdims=True)
      coord_weight = np.exp(-coord_dist/theta_p)
      color_weight = np.exp(-color_dist/theta_c)
      locallogits = pred[coord_indexes[0,:]]
      A = np.dot(coord_weight, locallogits)
      B = np.dot(color_weight.T, locallogits)
      newlogit = A*weight_p + B*weight_c
      o2[i , :] = newlogit
    pred = softmax(o2)
  return pred
...
def test(model1, model2, model3, data_loader, config, transform_data_fn=None, has_gt=True):
  ...
  with torch.no_grad():
    for iteration in range(max_iter):
      ...
      soutput1 = model1(*inputs)
      output1 = soutput1.F
      soutput2 = model2(*inputs)
      output2 = soutput2.F
      soutput3 = model3(*inputs)
      output3 = soutput3.F
      output = (output1 + output2 + output3) / 3
      #output = (output2 + output3) / 2
      #output = crf(output, coords, input, target)
      ...
      if iteration % config.test_stat_freq1 == 0 and iteration > 0:
