...
This file provides basic datasets definition for the datasets and some utility functions

TODO add docstring for the classes
...
class VoxelizationDataset(VoxelizationDatasetBase):
  ...
  """
    ...
    ROTATION_AUGMENTATION_BOUND = ((-np.pi / 6, np.pi / 6), (-np.pi, np.pi), (-np.pi / 6,
                                                                              np.pi / 6))
    ...
    def __getitem__(self, index):
        ...
        if self.PREVOXELIZATION_VOXEL_SIZE is not None:
            inds = ME.utils.sparse_quantize(coords / self.PREVOXELIZATION_VOXEL_SIZE,
                                            return_index=True)
            coords = coords[inds]
            feats = feats[inds]
            labels = labels[inds]
        ...
        coords, feats, labels, transformation = self.voxelizer.voxelize(coords,
                                                                        feats,
                                                                        labels,
                                                                        center=center)
...
class TemporalVoxelizationDataset(VoxelizationDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.PREVOXELIZATION_VOXEL_SIZE is not None:
            new_ptcs = []
            for ptc in ptcs:
                inds = ME.utils.sparse_quantize(ptc[:, :3] / self.PREVOXELIZATION_VOXEL_SIZE,
                                                return_index=True)
        ...
        outs = self.voxelizer.voxelize_temporal(coords,
                                                feats,
                                                labels,
                                                centers=centers,
        ...
        joint_coords = np.vstack([
            np.hstack((coords, np.ones((coords.shape[0], 1)) * i))
            for i, coords in enumerate(coords_t)
        ])
        ...
        # map labels not used for evaluation to ignore_label
        if self.input_transform is not None:
            joint_coords, joint_feats, joint_labels = self.input_transform(
                joint_coords, joint_feats, joint_labels)
        if self.target_transform is not None:
            joint_coords, joint_feats, joint_labels = self.target_transform(
                joint_coords, joint_feats, joint_labels)
        if self.IGNORE_LABELS is not None:
        ...
        if self.return_transformation:
            ...
            transformations = np.vstack([
                np.hstack((transformation, [i]))
                for i, transformation in enumerate(transformation_t)
            ])
...
def initialize_data_loader(dataset_class,
                           config,
                           phase, # TODO replace all 'phase' with 'split'
                           num_workers,
                           shuffle,
                           repeat,
                           augment_data,
    ...
    if augment_data:
        prevoxel_transform_train.append(t.ElasticDistortion(dataset_class.ELASTIC_DISTORT_PARAMS))
    ...
    if augment_data:
        input_transforms += [
            t.RandomDropout(0.2),
            t.RandomHorizontalFlip(dataset_class.ROTATION_AXIS, dataset_class.IS_TEMPORAL),
    ...
    if repeat:
        if get_world_size() > 1:
            data_args['sampler'] = DistributedInfSampler(
                dataset,
                shuffle=shuffle)  # torch.utils.data.distributed.DistributedSampler(dataset)
        else:
