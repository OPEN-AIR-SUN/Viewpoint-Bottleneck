...
This files defines ScannetVoxelizationDataset, which is a derived class of VoxelizationDataset.
ScannetVoxelizationDataset is used for training.
...
class ScannetVoxelizationDatasetBase(VoxelizationDataset):
    ...
    def test_pointcloud(self, pred_dir):
        ...
        for i, data_path in enumerate(self.data_paths):
            ...
            save_point_cloud(np.hstack(
                (pred[:, :3], np.array([SCANNET_COLOR_MAP[i] for i in pred[:, -1]]))),
            ...
            save_point_cloud(np.hstack(
                (query_xyz, np.array([SCANNET_COLOR_MAP[i] for i in ptc_pred]))),
...
class ScannetVoxelizationDataset(ScannetVoxelizationDatasetBase):
    DATA_ROOT = get_config().scannet_path
...
class ScannetVoxelizationtestDataset(ScannetVoxelizationDatasetBase):
    DATA_ROOT = get_config().scannet_test_path


class ScannetVoxelization2cmtestDataset(ScannetVoxelizationtestDataset):
    VOXEL_SIZE = 0.02