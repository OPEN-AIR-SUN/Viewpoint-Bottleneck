...
import glob
import numpy as np
from utils import read_plyfile, save_point_cloud
...
def handle_process(path):
    ...
    # Load label file.
    label_f = f.parent.parent.parent.parent / ('scan_label') / (
        f.parent.parent.stem + '/' + f.parent.stem + '/' + f.stem + '.labels' + f.suffix)
    if label_f.is_file():
